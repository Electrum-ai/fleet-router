...
# Keyword maps: tag → list of regex patterns
...
class TaskClassifier:
    ...
    def classify(self, prompt: str) -> tuple[str, float]:
        ...
        uncertainty = sum(1 for p in UNCERTAINTY_MARKERS if re.search(p, prompt_lower))
        ...
        if self._model and self._tag_embeddings:
            ...
            for tag, tag_emb in self._tag_embeddings.items():
                sim = float(np.dot(prompt_emb, tag_emb) / (np.linalg.norm(prompt_emb) * np.linalg.norm(tag_emb)))
                scores[tag] = max(scores[tag], sim * 0.8)

        # Pick best tag
        best_tag = max(scores, key=scores.get, default="general")
